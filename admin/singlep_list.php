<?php
    ...
    $sql="select count(*) as count from {$pre}single_page";
    $count= getOne($sql);
    $count=$count['count'];
 ...
 // 每页多少条数据
    ...
    if($count){
        ...
        foreach($data as $key=>$value){
            $data[$key]['content'] = cutstr($value['content'], 0, 20, '...');
         }
    }
                            ...
                            <tbody>
                                <?php if($count){ foreach ($data as $key => $value){ ?>
                                    ...
                                    <tr id="trparent">
                                        ...
                                        <!-- 构造序列号 不够指定长度补0 -->
                                        <td><?php echo str_pad($value['single_id'], 4, 0,  STR_PAD_LEFT) ?></td>
                                        <td><?php echo $value['name'] ?></td>
                                        <td class="text-nowrap" title="<?php echo $value['name'] ?>"><?php echo $value['content'] ?></td>  
                                         
                                        <td>
                                            <a class="btn btn-primary rounded" href="./singlep_add_edit.php?single_id=<?php echo $value['single_id']   ?>">
                                                <i class="fa fa-edit"></i>
                                                编辑
                                            </a>
                                            <button class="btn btn-danger rounded dlt" data-toggle="modal" data-target="#modal-delete" data-did="<?php echo $value['single_id'] ?>">
